...
public class CtrlMetodoBiseccion extends TopBarController {
    ...
    private DoubleDataSet bisectionPointsDataSet;
    ...
    /**
     * Configura el gráfico para visualizar la función y el proceso de bisección
     */
    private void setupChart() {
        try {
            ...
            bisectionPointsDataSet = new DoubleDataSet("Puntos de Bisección");
    ...
    void ejecutar(ActionEvent event) {
        ...
        bisectionPointsDataSet.clearData();
        ...
        try {
            ...
            float erMax = Float.parseFloat(txtErMax.getText());
            ...
            // Añadir los conjuntos de datos para bisección y raíz si no están ya
            if (!chart.getDatasets().contains(bisectionPointsDataSet)) {
                chart.getDatasets().add(bisectionPointsDataSet);
                bisectionPointsDataSet.setStyle("strokeColor=" + COLORS[1] + ";strokeWidth=0;markerSize=5;markerType=CIRCLE");
            } else {
                bisectionPointsDataSet.clearData();
            }
            ...
            // Ejecutar el método de bisección directamente para mostrar las iteraciones en el datagrid
            MetodoBiseccion metodo = new MetodoBiseccion();
            metodo.metodoBisec(inf, tblMBiseccion, userFunction);

            // Mostrar el resultado final
            if (!tblMBiseccion.getItems().isEmpty()) {
                InfoMetodoBiseccion ultimaIteracion = tblMBiseccion.getItems().get(tblMBiseccion.getItems().size() - 1);
                ...
                // Ejecutar el método de bisección con animación para visualizar el proceso en la minigráfica
                ejecutarMetodoBiseccionConAnimacion(inf);
            }
    ...
    /**
     * Actualiza el gráfico con los puntos de bisección y la raíz
     */
    private void actualizarGraficoConResultados() {
        try {
            ...
            for (InfoMetodoBiseccion iter : tblMBiseccion.getItems()) {
                // Añadir puntos a, b y c al gráfico
                bisectionPointsDataSet.add(iter.getA(), evaluateFunction(iter.getA()));
                bisectionPointsDataSet.add(iter.getB(), evaluateFunction(iter.getB()));
                bisectionPointsDataSet.add(iter.getC(), evaluateFunction(iter.getC()));
            }
    ...
    /**
     * Ejecuta el método de bisección con animaciones visuales
     */
    private void ejecutarMetodoBiseccionConAnimacion(InfoMetodoBiseccion inf) {
        try {
            ...
            bisectionPointsDataSet.add(a, fa);
            bisectionPointsDataSet.add(b, fb);
            ...
            java.util.List<InfoMetodoBiseccion> iteraciones = new java.util.ArrayList<>();

            // Ejecutar el método de bisección y almacenar cada iteración
            MetodoBiseccion metodo = new MetodoBiseccion();
            metodo.metodoBisecAnimado(inf, iteraciones, userFunction);

            // Crear animaciones para cada iteración
            for (int i = 0; i < iteraciones.size(); i++) {
                InfoMetodoBiseccion iter = iteraciones.get(i);

                // Crear animación para esta iteración
                ParallelTransition iterationAnimation = new ParallelTransition();

                // Añadir punto c (punto medio)
                ...
                Timeline addPointAnimation = new Timeline(
                    new KeyFrame(Duration.ZERO, e -> {
                        bisectionPointsDataSet.add(c, fc);
                ...
                // No añadimos la iteración a la tabla porque ya lo hizo metodoBisec
                // Solo hacemos scroll a la fila correspondiente
                final int index = i;
                Timeline scrollToRowAnimation = new Timeline(
                    new KeyFrame(Duration.ZERO, e -> {
                        if (index < tblMBiseccion.getItems().size()) {
                            tblMBiseccion.scrollTo(index);
                        }
                    })
                );
            ...
            if (!iteraciones.isEmpty()) {
                InfoMetodoBiseccion ultimaIteracion = iteraciones.get(iteraciones.size() - 1);
                double raiz = ultimaIteracion.getC();
                double error = ultimaIteracion.getErAct();
                double valorFuncion = evaluateFunction(raiz);
